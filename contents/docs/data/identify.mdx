...
You'll notice that one of the parameters in the `identify` method is a `properties` object. This enables you to set [user properties](/docs/getting-started/user-properties), which can be used in PostHog features such as [filters](/docs/product-analytics/trends#filtering-events-based-on-properties) or [cohorts](/docs/data/cohorts). 
...
> Note that depending on your [persistance configuration](/docs/libraries/js#persistence), the anonymous ID may not be stored, and thus regenerated across sessions.
